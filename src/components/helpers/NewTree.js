    ...
    verifyAndDelete() {
        ...
        this._leaves.forEach(leaf => {
            ...
            while (actvNode !== _this.root) {
                ...
                const [endHr, endMin] = actvNode.elem.start.split(":").map(Number);
                ...
                } else if (actvLength > 90 && times[`${startHr * 60 + startMin}`] === undefined &&
...
class ScheduleTree extends Tree {
    constructor(courses) {
        ...
        courses.forEach(course => {
            ...
            courseT.root.parent = _this.root;
            _this.root.children.push(...courseT.root.children);
            _this.segments[course.course_code] = courseT;
        });
    ...
    generateTree() {
        ...
        Object.keys(this.segments).forEach(key => {
            const courseT = _this.segments[key];
